...
def display_course(course: CourseRecord, target_grade: float) -> None:
    print(_BAR)
    ...
    if course.groups:
        ...
        for group in course.groups:
            ...
            graded_pts = sum(a.score for a in group.assignments if a.is_graded and a.score is not None)
            possible_pts = sum(a.points_possible for a in group.assignments if a.is_graded and a.points_possible > 0)
            ...
            print(f"    {name_col} {weight_str:>6}  {group_pct:>6}  [{graded_count}/{total_count} graded]")
        print()
    ...
    already_covered = [(n, g) for n, g in needs if not n.is_impossible and n.min_score_needed <= 0]
    ...
    if already_covered:
        ...
        for need, group_name in already_covered:
            print(f"    {need.assignment_name[:40]:<40}  {need.points_possible:.0f} pts  [any score]")
...
def display_all(courses: list[CourseRecord], target_grade: float) -> None:
    if not courses:
        print("No active courses found.")
        return
    for course in courses:
        display_course(course, target_grade)
    print(_BAR)
